...
## 2b. Two things already set in the project

You do not need to do anything here. This is a note so they are not lost if the iOS
project is ever regenerated by Capacitor, which would put the template defaults back.

**`ITSAppUsesNonExemptEncryption` is set to false** in `Info.plist`. Without it, App
Store Connect asks the export compliance question on every single upload, and a build
with it unanswered will not reach TestFlight. The app uses nothing but standard
HTTPS, which is exempt, so false is the correct answer.

**`UIRequiredDeviceCapabilities` is `arm64`,** not the `armv7` the Capacitor template
ships. Every device that can run this is arm64, and arm64 is what Apple documents for
anything since iOS 11.
...
**Screenshots:** the six `ios69-*.png` files from `app/store/screenshots/`. They are
**1260 x 2736**, which is the 6.9 inch size Apple now requires for any app that runs
on iPhone. Apple scales that set down for every smaller phone, so one set is enough.

The older 6.7 inch size, 1290 x 2796, is no longer on Apple's accepted list and gets
rejected at upload. If you have an old copy of those files anywhere, do not use them.
